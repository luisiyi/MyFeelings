package gutierrezmorquecho.luis.myfeelings

import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.util.Log
import android.widget.Toast
import gutierrezmorquecho.luis.myfeelings.utilities.CustomBarDrawable
import gutierrezmorquecho.luis.myfeelings.utilities.CustomCircleDrawable
import gutierrezmorquecho.luis.myfeelings.utilities.Emociones
import gutierrezmorquecho.luis.myfeelings.utilities.JSONFile
...
class MainActivity : AppCompatActivity() {
    ...
    //Este método permite leer el archivo JSON y asignar las variables globales para cuando inicie la aplicación y se parta de los valores reales.
    fun fetchingData(){
        try {
    ...
    fun iconoMayoria(){
        if (happy>veryHappy && happy>neutral && happy>sad && happy>verysad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_sentiment_satisfied))
        }
        if (veryHappy>happy &&  veryHappy>neutral && veryHappy>sad && veryHappy>verysad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_sentiment_verysatisfied))
        }
        if (neutral>veryHappy && neutral>happy && neutral>sad && neutral>verysad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_sentiment_neutral))
        }
        if (sad>happy && sad>neutral && sad>veryHappy && sad>verysad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_sentiment_dissatisfied))
        }
        if (verysad>happy && verysad>neutral && verysad>sad && verysad>veryHappy){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_sentiment_verydissatisfied))
        }
    }
